...
class AmountType extends AbstractStructBase
{
     ...
     * Set Base value
     * @param mixed $base
     * @return \OtaInterface\Otaconnect\StructType\AmountType
     */
    ...
    public function setDiscount(array $discount = array())
    {
        foreach ($discount as $amountTypeDiscountItem) {
            ...
            if (!$amountTypeDiscountItem instanceof \OtaInterface\Otaconnect\StructType\Discount) {
                throw new \InvalidArgumentException(sprintf('The Discount property can only contain items of \OtaInterface\Otaconnect\StructType\Discount, "%s" given', is_object($amountTypeDiscountItem) ? get_class($amountTypeDiscountItem) : gettype($amountTypeDiscountItem)), __LINE__);
            }
        }
