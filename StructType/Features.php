...
class Features extends AbstractStructBase
{
     ...
     * @param \OtaInterface\Otaconnect\StructType\Feature[] $feature
     * @return \OtaInterface\Otaconnect\StructType\Features
     */
    public function setFeature(array $feature = array())
    {
        foreach ($feature as $featuresFeatureItem) {
            // validation for constraint: itemType
            if (!$featuresFeatureItem instanceof \OtaInterface\Otaconnect\StructType\Feature) {
                throw new \InvalidArgumentException(sprintf('The Feature property can only contain items of \OtaInterface\Otaconnect\StructType\Feature, "%s" given', is_object($featuresFeatureItem) ? get_class($featuresFeatureItem) : gettype($featuresFeatureItem)), __LINE__);
            }
        }
