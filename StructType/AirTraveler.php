...
class AirTraveler extends AirTravelerType
{
     ...
     * @param \OtaInterface\Otaconnect\StructType\Comment[] $comment
     * @return \OtaInterface\Otaconnect\StructType\AirTraveler
     */
    public function setComment(array $comment = array())
    {
        foreach ($comment as $airTravelerCommentItem) {
            // validation for constraint: itemType
            if (!$airTravelerCommentItem instanceof \OtaInterface\Otaconnect\StructType\Comment) {
                throw new \InvalidArgumentException(sprintf('The Comment property can only contain items of \OtaInterface\Otaconnect\StructType\Comment, "%s" given', is_object($airTravelerCommentItem) ? get_class($airTravelerCommentItem) : gettype($airTravelerCommentItem)), __LINE__);
            }
        }
