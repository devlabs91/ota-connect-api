...
class Comment extends ParagraphType
{
     ...
     * @param string $commentOriginatorCode
     * @return \OtaInterface\Otaconnect\StructType\Comment
     */
    ...
    public function setAuthorizedViewer(array $authorizedViewer = array())
    {
        foreach ($authorizedViewer as $commentAuthorizedViewerItem) {
            ...
            if (!$commentAuthorizedViewerItem instanceof \OtaInterface\Otaconnect\StructType\AuthorizedViewer) {
                throw new \InvalidArgumentException(sprintf('The AuthorizedViewer property can only contain items of \OtaInterface\Otaconnect\StructType\AuthorizedViewer, "%s" given', is_object($commentAuthorizedViewerItem) ? get_class($commentAuthorizedViewerItem) : gettype($commentAuthorizedViewerItem)), __LINE__);
            }
        }
