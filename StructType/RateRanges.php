...
class RateRanges extends AbstractStructBase
{
     ...
     * @param \OtaInterface\Otaconnect\StructType\RateRange[] $rateRange
     * @return \OtaInterface\Otaconnect\StructType\RateRanges
     */
    public function setRateRange(array $rateRange = array())
    {
        foreach ($rateRange as $rateRangesRateRangeItem) {
            // validation for constraint: itemType
            if (!$rateRangesRateRangeItem instanceof \OtaInterface\Otaconnect\StructType\RateRange) {
                throw new \InvalidArgumentException(sprintf('The RateRange property can only contain items of \OtaInterface\Otaconnect\StructType\RateRange, "%s" given', is_object($rateRangesRateRangeItem) ? get_class($rateRangesRateRangeItem) : gettype($rateRangesRateRangeItem)), __LINE__);
            }
        }
