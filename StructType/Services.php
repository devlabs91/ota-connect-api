...
class Services extends AbstractStructBase
{
     ...
     * @param \OtaInterface\Otaconnect\StructType\Service[] $service
     * @return \OtaInterface\Otaconnect\StructType\Services
     */
    public function setService(array $service = array())
    {
        foreach ($service as $servicesServiceItem) {
            // validation for constraint: itemType
            if (!$servicesServiceItem instanceof \OtaInterface\Otaconnect\StructType\Service) {
                throw new \InvalidArgumentException(sprintf('The Service property can only contain items of \OtaInterface\Otaconnect\StructType\Service, "%s" given', is_object($servicesServiceItem) ? get_class($servicesServiceItem) : gettype($servicesServiceItem)), __LINE__);
            }
        }
