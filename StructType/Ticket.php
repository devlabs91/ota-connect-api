...
class Ticket extends AbstractStructBase
{
     ...
     * - maxOccurs: 16
     * - minOccurs: 0
     * @var \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr[]
     */
     ...
     * @param \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr[] $conjunctionTicketNbr
     ...
     * @return \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr[]|null
     */
     ...
     * @param \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr[] $conjunctionTicketNbr
     * @return \OtaInterface\Otaconnect\StructType\Ticket
     */
    public function setConjunctionTicketNbr(array $conjunctionTicketNbr = array())
    {
        foreach ($conjunctionTicketNbr as $ticketConjunctionTicketNbrItem) {
            ...
            if (!$ticketConjunctionTicketNbrItem instanceof \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr) {
                throw new \InvalidArgumentException(sprintf('The ConjunctionTicketNbr property can only contain items of \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr, "%s" given', is_object($ticketConjunctionTicketNbrItem) ? get_class($ticketConjunctionTicketNbrItem) : gettype($ticketConjunctionTicketNbrItem)), __LINE__);
            }
        }
     ...
     * @throws \InvalidArgumentException
     * @param \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr $item
     * @return \OtaInterface\Otaconnect\StructType\Ticket
     */
    public function addToConjunctionTicketNbr(\OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr) {
            throw new \InvalidArgumentException(sprintf('The ConjunctionTicketNbr property can only contain items of \OtaInterface\Otaconnect\StructType\ConjunctionTicketNbr, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
