...
class Prices extends AbstractStructBase
{
     ...
     * Meta informations extracted from the WSDL
     * - maxOccurs: 9
     * @var \OtaInterface\Otaconnect\StructType\Price[]
     */
    public $Price;
    /**
     ...
     * @param \OtaInterface\Otaconnect\StructType\Price[] $price
    ...
    /**
     * Get Price value
     * @return \OtaInterface\Otaconnect\StructType\Price[]|null
     */
     ...
     * @param \OtaInterface\Otaconnect\StructType\Price[] $price
     * @return \OtaInterface\Otaconnect\StructType\Prices
     */
    public function setPrice(array $price = array())
    {
        foreach ($price as $pricesPriceItem) {
            // validation for constraint: itemType
            if (!$pricesPriceItem instanceof \OtaInterface\Otaconnect\StructType\Price) {
                throw new \InvalidArgumentException(sprintf('The Price property can only contain items of \OtaInterface\Otaconnect\StructType\Price, "%s" given', is_object($pricesPriceItem) ? get_class($pricesPriceItem) : gettype($pricesPriceItem)), __LINE__);
            }
        }
        $this->Price = $price;
        return $this;
     ...
     * @throws \InvalidArgumentException
     * @param \OtaInterface\Otaconnect\StructType\Price $item
     * @return \OtaInterface\Otaconnect\StructType\Prices
     */
    public function addToPrice(\OtaInterface\Otaconnect\StructType\Price $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \OtaInterface\Otaconnect\StructType\Price) {
            throw new \InvalidArgumentException(sprintf('The Price property can only contain items of \OtaInterface\Otaconnect\StructType\Price, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
        $this->Price[] = $item;
        return $this;
    }
